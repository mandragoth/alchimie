...
package void loadAlchimieLibDrawable(GrLibDefinition library) {
    ...
    version (AlmaRuna) {
        library.addProperty(&_getPosition, &_setPosition, "position", entityType, vec2Type);
    }
    ...
    version (AlmaMagia) {
        ...
        // Instance operations
        library.addFunction(&_getGlobalPosition3D, "globalPosition", [
                instanceType
            ], [vec3Type]);
        library.addFunction(&_getLocalPosition3D, "localPosition", [
                instanceType
            ], [vec3Type]);
        library.addFunction(&_setPosition2D, "position", [
                instanceType, vec2Type
            ]);
        library.addFunction(&_setPosition3D, "position", [
                instanceType, vec3Type
            ]);
        library.addFunction(&_setRotation2D, "rotation", [instanceType, grFloat]);
        library.addFunction(&_setRotation3D, "rotation", [
                instanceType, vec3Type
            ]);
        ...
        library.addFunction(&_setDisplayBoneId, "displayBoneId", [
                modelType, grInt
            ], []);
    }
}
...
private void _newSprite(GrCall call) {
    Sprite sprite = Kernel.res.get!Sprite(call.getString(0));
...
version (AlmaRuna) {
    private void _getPosition(GrCall call) {
        ...
        call.setNative(toSVec2f(entity.position));
    }

    private void _setPosition(GrCall call) {
        Entity entity = call.getNative!Entity(0);
        entity.position = cast(vec2) call.getNative!SVec2f(1);
    }
}
...
private void _newDirectionalLight(GrCall call) {
    DirectionalLight directionalLight = new DirectionalLight();
        ...
        Kernel.light.directionalLight = directionalLight;
    ...
    private void _newPointLight(GrCall call) {
        ...
        // Register light in the renderer
        Kernel.light.addPointLight(pointLight);
    ...
    private void _newSpotLight(GrCall call) {
        ...
        // Register light in the renderer
        Kernel.light.addSpotLight(spotLight);
