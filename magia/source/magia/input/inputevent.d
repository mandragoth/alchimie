...
final class InputEvent {
    ...
    private {
        ...
        union {
            KeyButtonEvent _keyButton;
            MouseButtonEvent _mouseButton;
            MouseMotionEvent _mouseMotion;
            MouseWheelEvent _mouseWheel;
            TextInputEvent _textInput;
            FileEvent _dropFile;
        }
    }
    ...
    /// Init
    this(KeyButtonEvent event) {
        _type = Type.keyButton;
        _keyButton = event;
    }
    ...
    this(const InputEvent event) {
        ...
        case none:
            break;
        case keyButton:
            _keyButton = event._keyButton;
            break;
        case mouseButton:
            _mouseButton = event._mouseButton;
            break;
        case mouseMotion:
            _mouseMotion = event._mouseMotion;
            break;
        case mouseWheel:
            _mouseWheel = event._mouseWheel;
            break;
        case textInput:
            _textInput = event._textInput;
            break;
        case dropFile:
            _dropFile = event._dropFile;
            break;
        }
    }
    ...
    static {
        ...
        InputEvent keyButton(KeyButton button, bool pressed, bool isEcho) {
            InputEvent event = new InputEvent;
            event._type = Type.keyButton;
            event._isAccepted = false;
            event._keyButton.button = button;
            event._keyButton.pressed = pressed;
            event._keyButton.isEcho = isEcho;
            return event;
        }

        /// Retourne un événement correspondant à une touche de la souris
        InputEvent mouseButton(MouseButton button, bool pressed, uint clicks,
            vec2i globalPosition, vec2i position) {
            InputEvent event = new InputEvent;
            event._type = Type.mouseButton;
            event._isAccepted = false;
            event._mouseButton.button = button;
            event._mouseButton.pressed = pressed;
            event._mouseButton.clicks = clicks;
            event._mouseButton.globalPosition = globalPosition;
            event._mouseButton.position = position;
            return event;
        }
        ...
        InputEvent mouseMotion(vec2i globalPosition, vec2i position) {
            ...
            event._isAccepted = false;
            event._mouseMotion.globalPosition = globalPosition;
            event._mouseMotion.position = position;
            return event;
        }
